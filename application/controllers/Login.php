...
class Login extends CI_Controller
{
	...
	public function cek_login()
	{
		...
		if ($ceklogin->num_rows() == 1) {
			...
			$this->session->set_userdata('personvalid', $data['krit1'] . $data['per1']);
			$this->session->set_userdata('personrilis', $data['krit2'] . $data['per2']);
			$this->session->set_userdata('infovalid', $this->m_login->adavalidator($data['kritkar'] . $data['person_id']));
