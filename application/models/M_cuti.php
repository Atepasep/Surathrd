...
class M_cuti extends CI_Model
{
	...
	{
		$datavalid = $this->session->userdata('infovalid');
		$query = $this->db->query("SELECT a.*,b.valid,b.rilis,b.nama,b.bagian,b.grp,c.keterangan as keterangan FROM cuti a
		LEFT JOIN mperson b ON CONCAT(b.kritkar,b.person_id) = CONCAT(a.kritkar,a.person_id)
		LEFT JOIN jeniscuti c ON c.kode = a.jncuti
		WHERE (valid=$datavalid OR rilis=$datavalid) AND if(valid=$datavalid,appcol=0,approve=0 AND appcol=1)");
		return $query->result_array();
	}
	...
	{
		$datavalid = $this->session->userdata('infovalid');
		$query = $this->db->query("SELECT a.*,b.valid,b.rilis,b.nama,b.bagian,b.grp,c.keterangan as keterangan FROM izin a
		LEFT JOIN mperson b ON CONCAT(b.kritkar,b.person_id) = CONCAT(a.kritkar,a.person_id)
		LEFT JOIN jeniscuti c ON c.kode = a.jnizin
		WHERE (valid=$datavalid OR rilis=$datavalid) AND if(valid=$datavalid,appcol=0,approve=0 AND appcol=1)");
		return $query->result_array();
	}
	...
	public function simpancuti()
	{
		...
		$spc = getspc($this->session->userdata('kritper'));
